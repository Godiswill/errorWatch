...
describe('ErrorWatch', function () {
  ...
  describe('General', function () {
    it('should not remove anonymous functions from the stack', function () {
      ...
      const mock_err = { stack: stack_str };
      const stackFrames = ErrorWatch.computeStackTrace.computeStackTraceFromStackProp(mock_err);

      // Make sure ErrorWatch didn't remove the anonymous functions
  ...
  describe('.computeStackTrace', function () {
    it('should handle a native error object', function () {
      ...
      const stack = ErrorWatch.computeStackTrace(ex);
    ...
    it('should handle a native error object stack from Chrome', function () {
      ...
      const stackFrames = ErrorWatch.computeStackTrace(mockErr);
      expect(stackFrames).toBeTruthy();
  ...
  describe('error notifications', function () {
    ...
    let oldOnErrorHandler;

    // ErrorWatch waits 2000ms for window.onerror to fire, so give the tests
    // some extra time.
    //this.timeout(3000);
    ...
    beforeEach(function () {
      ...
      // we can't do that without clobbering ErrorWatch's handler, which can only
      // be installed once.
    ...
    afterEach(function () {
      ...
      if (subscriptionHandler) {
        ErrorWatch.report.unsubscribe(subscriptionHandler);
    ...
    describe('with undefined arguments', function () {
      it('should pass undefined:undefined', function (done) {
        ...
        ErrorWatch.report.subscribe(subscriptionHandler);
    ...
    function testErrorNotification(collectWindowErrors, callOnError, numReports, done) {
      let numDone = 0;
      // ErrorWatch's collectWindowErrors flag shouldn't affect direct calls
      // to ErrorWatch.report, so we parameterize it for the tests.
      ErrorWatch.collectWindowErrors = collectWindowErrors;
      ...
      // ErrorWatch.report always throws an exception in order to trigger
      ...
      //
      // We test multiple reports because ErrorWatch has special logic for when
  ...
  describe('globalHandlers', function () {
    let oldOnErrorHandler;
    let oldOnUnhandledRejectionHandler;

    let onErrorHandler;
    let onUnhandledRejectionHandler;
    ...
    describe('cleanup after unsubscribing', function () {
      ...
      it('should should restore original `window.onerror` once unsubscribed', function () {
        ...
        ErrorWatch.report.subscribe(testHandler);
        expect(window.onerror).not.toBe(onErrorHandler);

        ErrorWatch.report.unsubscribe(testHandler);
        expect(window.onerror).toBe(onErrorHandler);
      });
