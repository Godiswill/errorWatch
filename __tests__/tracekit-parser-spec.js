...
describe('Parser', function () {
  ...
  const CapturedExceptions = require('./fixtures/captured-errors');
  ...
  function baz() {
    return ErrorWatch.computeStackTrace.ofCaller();
  }
  ...
  it('should parse Safari 6 error', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.SAFARI_6);
  ...
  it('should parse Safari 7 error', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.SAFARI_7);
    expect(stackFrames).toBeTruthy();
  ...
  it('should parse Safari 8 error', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.SAFARI_8);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(3);
  ...
  it('should parse Safari 8 eval error', function () {
    ...
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.SAFARI_8_EVAL);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(3);
  ...
  it('should parse Firefox 3 error', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.FIREFOX_3);
    expect(stackFrames).toBeTruthy();
  ...
  it('should parse Firefox 7 error', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.FIREFOX_7);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(7);
  ...
  it('should parse Firefox 14 error', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.FIREFOX_14);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(3);
    expect(stackFrames.stack[0]).toEqual({ url: 'http://path/to/file.js', func: '?', args: [], line: 48, column: null, context: null });
    expect(stackFrames.stack[1]).toEqual({ url: 'http://path/to/file.js', func: 'dumpException3', args: [], line: 52, column: null, context: null });
  ...
  it('should parse Firefox 31 error', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.FIREFOX_31);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(3);
  ...
  it('should parse Firefox 44 ns exceptions', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.FIREFOX_44_NS_EXCEPTION);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(4);
  ...
  it('should parse Chrome error with no location', function () {
    const stackFrames = ErrorWatch.computeStackTrace({ stack: "error\n at Array.forEach (native)" });
  ...
  it('should parse Chrome 15 error', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.CHROME_15);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(4);
  ...
  it('should parse Chrome 36 error with port numbers', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.CHROME_36);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(3);
  ...
  it('should parse Chrome error with webpack URLs', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.CHROME_XX_WEBPACK);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(4);
  ...
  it('should parse nested eval() from Chrome', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.CHROME_48_EVAL);
    expect(stackFrames).toBeTruthy();
  ...
  it('should parse Chrome error with blob URLs', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.CHROME_48_BLOB);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(7);
  ...
  it('should parse empty IE 9 error', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.IE_9);
    expect(stackFrames).toBeTruthy();
    stackFrames.stack && expect(stackFrames.stack.length).toBe(0);
  });

  it('should parse IE 10 error', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.IE_10);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(3);
  ...
  it('should parse IE 11 error', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.IE_11);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(3);
    // TODO: func should be normalized
  ...
  it('should parse IE 11 eval error', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.IE_11_EVAL);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(3);
  ...
  it('should parse Opera 8.54 error', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.OPERA_854);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(7);
  ...
  it('should parse Opera 9.02 error', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.OPERA_902);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(7);
    expect(stackFrames.stack[0]).toEqual({ url: 'http://path/to/file.js', func: '?', args: [], line: 44, column: null, context: ['    this.undef();'] });
    expect(stackFrames.stack[1]).toEqual({ url: 'http://path/to/file.js', func: '?', args: [], line: 31, column: null, context: ['    ex = ex || this.createException();'] });
  ...
  it('should parse Opera 9.27 error', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.OPERA_927);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(3);
  ...
  it('should parse Opera 9.64 error', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.OPERA_964);
    expect(stackFrames).toBeTruthy();
  ...
  it('should parse Opera 10 error', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.OPERA_10);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(7);
  ...
  it('should parse Opera 11 error', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.OPERA_11);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(7);
  ...
  it('should parse Opera 12 error', function () {
    ...
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.OPERA_12);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(3);
  ...
  it('should parse Opera 25 error', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.OPERA_25);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(3);
    expect(stackFrames.stack[0]).toEqual({ url: 'http://path/to/file.js', func: '?', args: [], line: 47, column: 22, context: null });
    expect(stackFrames.stack[1]).toEqual({ url: 'http://path/to/file.js', func: 'foo', args: [], line: 52, column: 15, context: null });
  ...
  it('should parse PhantomJS 1.19 error', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.PHANTOMJS_1_19);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(3);
  ...
  it('should parse Firefox errors with resource: URLs', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.FIREFOX_50_RESOURCE_URL);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(3);
  ...
  it('should parse Firefox errors with eval URLs', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.FIREFOX_43_EVAL);
    expect(stackFrames).toBeTruthy();
    expect(stackFrames.stack.length).toBe(5);
  ...
  it('should parse React Native errors on Android', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.ANDROID_REACT_NATIVE);
    expect(stackFrames).toBeTruthy();
  ...
  it('should parse React Native errors on Android Production', function () {
    const stackFrames = ErrorWatch.computeStackTrace(CapturedExceptions.ANDROID_REACT_NATIVE_PROD);
    expect(stackFrames).toBeTruthy();
